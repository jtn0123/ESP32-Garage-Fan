...
describe("build", () => {
  ...
  it("pads every series to temp_c length so short columns cannot plot NaN", () => {
    ...
    const s = build(
      h({ hpa: [1010], out_f: [70], spd: [1], batt_v: [3.9], chg: [1] }),
    );
    expect(s.n).toBe(3);
...
describe("build: the row spacing is measured, not assumed", () => {
  ...
  it("falls back to interval_s when no stamp has synced", () => {
    const n = 4;
    expect(
      build(h({ ts: [0, 0, 0, 0].slice(0, n), temp_c: [24, 24, 24, 24] })).step,
    ).toBe(300);
  });
});
...
describe("tail", () => {
  ...
  it("slices every series to the same length", () => {
    const h = at(288, 1_700_000_000, 300);
    const six = tail(h, 6);
  ...
  it("falls back to the cadence when the clock has not synced", () => {
    ...
    const six = tail(h, 6);
    expect(six.ts.length).toBe(72); // 6 h / 5 min, counted rather than timed
  });
