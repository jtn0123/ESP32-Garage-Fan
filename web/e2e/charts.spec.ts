 ...
 * purpose. So the thing worth pinning here is the request: a 6 H button that
 * sent days=0.25 would get a 400 and draw nothing, and a 6 H button that sent
 * days=6 would silently draw six DAYS under a six-hour title. Neither failure
 * is visible in a unit test, which never clicks anything. The slicing maths
 * itself is covered in tests/series.test.ts.
 */
for (const [d, hours] of [
  ...
  test(`the ${hours}H range windows the 24 h response instead of asking for it`, async ({
    ...
    await expect(page.locator(`#ranges button[data-d="${d}"]`)).toHaveClass(
      /on/,
    );
    await expect(page.locator("#chtitle")).toHaveText(
      new RegExp(`${hours} HOURS`, "i"),
    );
    await expect(page.locator("#ranges button.on")).toHaveCount(1);
    await expect
      .poll(() => fetches.some((r) => /days=1\b/.test(r.url())))
      .toBeTruthy();
    // Never the fraction, and never the number of hours read as days.
    expect(fetches.some((r) => /days=(0\.|6\b|12\b)/.test(r.url()))).toBe(
      false,
    );
  });
  ...
  test(`the ${hours}H range actually plots its data`, async ({ page }) => {
    ...
    await expect(page.locator("#tcap")).not.toContainText(
      /could not|no samples/i,
    );
  });
}

test("leaving a sub-day range for a wide one asks the device again", async ({
  page,
}) => {
  ...
  await expect
    .poll(() => fetches.some((r) => /days=7\b/.test(r.url())))
    .toBeTruthy();
});
...
for (const [days, title] of [
  ...
  test(`selecting ${days}d refetches that range and relabels the chart`, async ({
    page,
  }) => {
    await openConsole(page);
    const fetches = recordRequests(page, /\/api\/history/);
    await page.locator(`#ranges button[data-d="${days}"]`).click();
    await expect(page.locator(`#ranges button[data-d="${days}"]`)).toHaveClass(
      /on/,
    );
    ...
    await expect
      .poll(() =>
        fetches.some((r) => new RegExp(`days=${days}\\b`).test(r.url())),
      )
      .toBeTruthy();
  });
}
...
test("every series has a chip", async ({ page }) => {
  ...
  for (const { key } of CHIP_ROWS) {
    await expect(
      page.locator(`#chips button[data-key="${key}"]`),
    ).toBeVisible();
  }
});
...
for (const { key, row } of CHIP_ROWS) {
  test(`the ${key} chip toggles ${row} and nothing else`, async ({ page }) => {
    ...
    const shownBefore = !((await target.getAttribute("class")) ?? "").includes(
      "hide",
    );
    ...
    expect(await otherState(), `toggling ${key} disturbed another row`).toEqual(
      before,
    );
...
test("a scrubbed moment on a multi-day range carries its date", async ({
  page,
}) => {
  await openConsole(page);
  await page.locator('#ranges button[data-d="7"]').click();
...
test("a status bit toggles on click", async ({ page }) => {
  ...
  await bit.click();
  expect(await hidden(), "a second click did not toggle the tip back").toBe(
    !afterFirst,
  );
  await bit.click();
...
test("the VOC row shows data and a labelled readout", async ({ page }) => {
  ...
  await expect(page.locator("#roV")).toContainText(/index \d+/, {
    timeout: 15_000,
  });
...
test("a controller with no restart marks still draws its charts", async ({
  page,
}) => {
  ...
  await page.route("**/api/boots*", (r) =>
    r.fulfill({ status: 404, body: '{"error":"nope"}' }),
  );
  await openConsole(page);
...
for (const width of [320, 412] as const) {
  for (const days of ["1", "7", "30"] as const) {
    test(`starting a scrub moves nothing at ${width}px on the ${days}-day range`, async ({
      page,
    }) => {
      ...
      const shape = () =>
        page.evaluate(() => {
          ...
          return {
            ...
            heroCols: [...hero.children].map(
              (c) => +(c.getBoundingClientRect().top + scrollY).toFixed(1),
            ),
            reason: box("#reason"),
            chead: box("#chead"),
            stampW: box("#stamp")!.w,
      ...
      expect(after.stampW, "the hero badge changed width on scrub").toBe(
        before.stampW,
      );
      expect(after.hero, "the hero re-measured on scrub").toEqual(before.hero);
      expect(after.heroCols, "a hero column moved on scrub").toEqual(
        before.heroCols,
      );
      expect(after.reason, "the state sentence re-measured on scrub").toEqual(
        before.reason,
      );
      expect(after.chead, "the sticky readout re-measured on scrub").toEqual(
        before.chead,
      );
      expect(after.plotTop, "the plot moved under the scrub").toBe(
        before.plotTop,
      );
    });
  }
...
test("the row chips are thumb-sized and say the strip continues", async ({
  page,
}) => {
  ...
  for (const i of [0, 3, 6]) {
    ...
    expect(b!.height, `chip ${i} is under 44 px tall`).toBeGreaterThanOrEqual(
      44,
    );
  }
  ...
  expect(
    inView,
    "NOX is still not on screen after scrolling to the end",
  ).toBeTruthy();
...
test("the range chips are thumb-sized and separated", async ({ page }) => {
  ...
  // Compared within a row only: the strip wraps at six ranges, and the first
  // chip of a new line legitimately sits left of the last one above it.
